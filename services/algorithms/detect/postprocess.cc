/*
 * 取自 rknn_model_zoo / yolov8/cpp/postprocess.cc，去掉 RV1106/RKNPU1 分支与 file I/O 依赖。
 * COCO 80 类别名以静态字符串数组的方式编译进二进制，免去板端再去拷贝
 * coco_80_labels_list.txt 的运维麻烦。
 */
#include "yolov8.h"
...
extern "C" int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box,
...
extern "C" int init_post_process(void)
{
    /* COCO labels 已在编译期嵌入；保留接口语义与原 demo 对齐。 */
    return 0;
}

extern "C" void deinit_post_process(void)
{
    /* nothing to free */
}

extern "C" const char *coco_cls_to_name(int cls_id)
{
    if (cls_id < 0 || cls_id >= OBJ_CLASS_NUM) {
        return "null";
    }
    const char *name = kCocoLabels[cls_id];
