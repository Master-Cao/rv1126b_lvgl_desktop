...
extern "C" int init_yolov8_model(const char *model_path, rknn_app_context_t *app_ctx)
{
    int ret;
    int model_len = 0;
    char *model = NULL;
    ...
    if (!model || model_len <= 0) {
        printf("init_yolov8_model: load_model fail: %s\n", model_path);
        return -1;
    }
    ...
    free(model);
    if (ret < 0) {
        printf("init_yolov8_model: rknn_init fail! ret=%d\n", ret);
        return -1;
    }
    ...
    if (ret != RKNN_SUCC) {
        printf("init_yolov8_model: rknn_query IO_NUM fail! ret=%d\n", ret);
        rknn_destroy(ctx);
        return -1;
    }
    printf("yolov8 model: input num=%d, output num=%d\n", io_num.n_input, io_num.n_output);
    ...
    for (uint32_t i = 0; i < io_num.n_input; i++) {
        ...
        if (ret != RKNN_SUCC) {
            printf("init_yolov8_model: rknn_query INPUT_ATTR fail! ret=%d\n", ret);
            rknn_destroy(ctx);
            return -1;
        }
    ...
    for (uint32_t i = 0; i < io_num.n_output; i++) {
        ...
        if (ret != RKNN_SUCC) {
            printf("init_yolov8_model: rknn_query OUTPUT_ATTR fail! ret=%d\n", ret);
            rknn_destroy(ctx);
            return -1;
        }
    ...
    if (input_attrs[0].fmt == RKNN_TENSOR_NCHW) {
        printf("yolov8 model: NCHW input fmt\n");
        app_ctx->model_channel = input_attrs[0].dims[1];
        app_ctx->model_height = input_attrs[0].dims[2];
        app_ctx->model_width = input_attrs[0].dims[3];
    } else {
        printf("yolov8 model: NHWC input fmt\n");
        app_ctx->model_height = input_attrs[0].dims[1];
        app_ctx->model_width = input_attrs[0].dims[2];
        app_ctx->model_channel = input_attrs[0].dims[3];
    }
    printf("yolov8 model: input height=%d, width=%d, channel=%d, is_quant=%d\n",
           app_ctx->model_height, app_ctx->model_width, app_ctx->model_channel, app_ctx->is_quant);
    return 0;
}

extern "C" int release_yolov8_model(rknn_app_context_t *app_ctx)
{
...
extern "C" int inference_yolov8_model(rknn_app_context_t *app_ctx, image_buffer_t *img,
                                      object_detect_result_list *od_results)
{
    ...
    if (!dst_img.virt_addr) {
        printf("inference_yolov8_model: malloc letterbox buffer %d fail\n", dst_img.size);
        return -1;
    }

    ret = convert_image_with_letterbox_cpu(img, &dst_img, &letter_box, (unsigned char)bg_color);
    if (ret < 0) {
        printf("inference_yolov8_model: letterbox fail ret=%d\n", ret);
    ...
    if (ret < 0) {
        printf("inference_yolov8_model: rknn_inputs_set fail ret=%d\n", ret);
        free(dst_img.virt_addr);
        return -1;
    }
    ...
    if (ret < 0) {
        printf("inference_yolov8_model: rknn_run fail ret=%d\n", ret);
        free(dst_img.virt_addr);
        return -1;
    }
    ...
    if (ret < 0) {
        printf("inference_yolov8_model: rknn_outputs_get fail ret=%d\n", ret);
        free(dst_img.virt_addr);
        return ret;
    }
