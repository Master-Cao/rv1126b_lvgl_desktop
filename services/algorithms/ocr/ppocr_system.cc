...
/* file_utils.h / image_utils.h 不再依赖：
 *   read_data_from_file -> 本文件内联实现
 *   convert_image / get_image_size -> 用 CPU 双线性 resize 替代
 */
...
/* CPU 双线性 resize，仅支持 IMAGE_FORMAT_RGB888；
 * 对应原 ppocr 的 convert_image(src, dst, NULL, NULL, 0)。 */
static int convert_image(image_buffer_t* src, image_buffer_t* dst,
                         image_rect_t* /*src_box*/, image_rect_t* /*dst_box*/, char /*color*/)
{
    if (!src || !dst || src->format != IMAGE_FORMAT_RGB888 || dst->format != IMAGE_FORMAT_RGB888) {
        return -1;
    }
    if (!src->virt_addr || !dst->virt_addr || src->width <= 0 || src->height <= 0 ||
    ...
    cv::resize(src_mat, dst_mat, cv::Size(dst->width, dst->height), 0, 0, cv::INTER_LINEAR);
    return 0;
}
