...
extern "C" int seg_post_process(rknn_app_context_t *app_ctx, rknn_output *outputs, letterbox_t *letter_box,
                                float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    uint8_t *matmul_out = (uint8_t *)malloc((size_t)boxes_num * PROTO_HEIGHT * PROTO_WEIGHT);
    if (!matmul_out) {
        return -1;
    }
    matmul_by_cpu_uint8(filterSegments_by_nms, proto, matmul_out, ROWS_A, COLS_A, COLS_B);

    uint8_t *seg_mask = (uint8_t *)malloc((size_t)boxes_num * model_in_height * model_in_width);
    if (!seg_mask) {
        free(matmul_out);
        return -1;
    }
    ...
    uint8_t *all_mask_in_one = (uint8_t *)malloc((size_t)model_in_height * model_in_width);
    if (!all_mask_in_one) {
        free(seg_mask);
        free(matmul_out);
        return -1;
    }
    memset(all_mask_in_one, 0, (size_t)model_in_height * model_in_width);
    ...
    uint8_t *cropped_seg_mask = (uint8_t *)malloc((size_t)cropped_height * cropped_width);
    uint8_t *real_seg_mask = (uint8_t *)malloc((size_t)ori_in_height * ori_in_width);
    if (!cropped_seg_mask || !real_seg_mask) {
...
extern "C" int seg_init_post_process(void)
{
    return 0;
}

extern "C" const char *seg_coco_cls_to_name(int cls_id)
{
...
extern "C" void seg_deinit_post_process(void)
{
}
